	...
	return new Promise<UniUpgradeCenterResult>((resolve, reject) => {
        ...
        if (code > 0) {
          ...
          uni.openDialogPage({
            ...
            disableEscBack: true,
            fail: (err) => {
              __f__('error','at uni_modules/uni-upgrade-center-app/utils/check-update.ts:85','更新弹框跳转失败', err)
          ...
          __f__('error','at uni_modules/uni-upgrade-center-app/utils/check-update.ts:93',message)
			...
			uni.downloadFile({
				...
				success: res => {
					if (res.statusCode !== 200) {
						__f__('error','at uni_modules/uni-upgrade-center-app/utils/check-update.ts:156','下载安装包失败');
						return;
					}
					// 下载好直接安装，下次启动生效
