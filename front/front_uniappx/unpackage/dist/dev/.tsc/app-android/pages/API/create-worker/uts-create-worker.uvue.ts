  ...
  const __sfc__ = defineComponent({
    ...
    methods: {
      // 添加日志方法
        ...
        const logItem = { __$originalPosition: new UTSSourceMapPosition("logItem", "pages/API/create-worker/uts-create-worker.uvue", 70, 15), 
...
function GenPagesAPICreateWorkerUtsCreateWorkerRender(this: InstanceType<typeof __sfc__>): any | null {
const _ctx = this
const _cache = this.$.renderCache
  return createElementVNode("scroll-view", utsMapOf({ class: "container" }), [
    ...
    createElementVNode("view", utsMapOf({ class: "button-group" }), [
      createElementVNode("text", utsMapOf({ class: "description-text" }), "操作步骤：1.创建Worker 2.发送数据测试（消息监听已在创建Worker时添加）"),
      createElementVNode("button", utsMapOf({
        class: "btn",
        type: "primary",
        disabled: _ctx.created_boolean,
        onClick: _ctx.create
      }), "UTS插件中创建Worker", 8 /* PROPS */, ["disabled", "onClick"]),
      createElementVNode("button", utsMapOf({
        class: "btn",
        onClick: _ctx.destory,
        disabled: _ctx.workerStatus != 'created'
      }), "UTS插件中销毁Worker", 8 /* PROPS */, ["onClick", "disabled"])
    ]),
    createElementVNode("view", utsMapOf({ class: "input-section" }), [
      ...
      createElementVNode("button", utsMapOf({
        ...
        disabled: _ctx.workerStatus != 'created'
      }), "通过UTS插件发送到WorkerTask (值+1)", 8 /* PROPS */, ["onClick", "disabled"])
    ]),
