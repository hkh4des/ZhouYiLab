          ...
          this.uniVerifyManager?.customLogin({
            ...
            fail: (error) => {
              if (error.errCode == 40001) {
                ...
                const errorMsg = UTSAndroid.consoleDebugError(JSON.parseObject(error.cause?.cause?.message ?? ""), " at pages/API/get-uni-verify-manager/uni-verify-custom-page.uvue:109")?.getString("errorDesc") ?? error.errMsg;
                uni.showToast({
                  title: errorMsg,
                  position: "bottom",
                  duration: 2000
