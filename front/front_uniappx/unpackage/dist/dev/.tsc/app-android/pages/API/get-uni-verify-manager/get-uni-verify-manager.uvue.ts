      ...
      preLogin(callback: (() => void)) {
        this.uniVerifyManager?.preLogin({
          ...
          fail: (err) => {
            ...
            uni.showModal({
              title: '预登录失败',
              content: hasCauseMessage ? UTSAndroid.consoleDebugError(JSON.parseObject(err.cause?.cause?.message ?? ''), " at pages/API/get-uni-verify-manager/get-uni-verify-manager.uvue:103")?.getString("errorDesc") : err.errMsg,
              showCancel: false
            });
          }
        });
      ...
      login(fullScreen : boolean) {
        this.uniVerifyManager?.login({
          ...
          fail: (err) => {
            ...
            uni.showModal({
              title: '登录失败',
              content: hasCauseMessage ? UTSAndroid.consoleDebugError(JSON.parseObject(err.cause?.cause?.message ?? ""), " at pages/API/get-uni-verify-manager/get-uni-verify-manager.uvue:125")?.getString("errorDesc") : err.errMsg,
              showCancel: false
            });
          }
        });
